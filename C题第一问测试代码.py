...
# 5. 面积约束：如果某地块某年种植某作物，则种植面积必须大于 0 for i in plots:
for j in crops:
    for t in years:
...
# 创建一个空的结果列表来存储地块名称、年份、作物名称和种植面积
results = []
        ...
        for t in years:
            if x[i['地块名称'], j['作物名称'], t].value() > 0:
                results.append({
                    ...
                    '年份': t,
                    '作物名称': j['作物名称'],
                    '种植面积': x[i['地块名称'], j['作物名称'], t].value()
                })

# 将结果转换为 DataFrame
df_results = pd.DataFrame(results)

# 按地块名称、年份、作物名称分组，并累加种植面积
df_results_grouped = df_results.groupby(['地块名称', '年份', '作物名称'], as_index=False)['种植面积'].sum()

# 将结果导出到 Excel 文件
df_results_grouped.to_excel('优化结果.xlsx', index=False)

print("结果已导出到 '优化结果.xlsx'")
